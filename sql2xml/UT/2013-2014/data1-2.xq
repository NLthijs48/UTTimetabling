...
return <day>{$day//dategiven}{$day//daygiven}{
  ...
  return 
    ...
    return <set><name>{$studentset}</name>
      <duration>
        {fn:minutes-from-duration(xs:time($activity/endtime)-xs:time($activity/starttime))+60*fn:hours-from-duration(xs:time($activity/endtime)-xs:time($activity/starttime))}
      </duration>
      <endtime>{fn:minutes-from-time(xs:time($activity/endtime))+60*fn:hours-from-time(xs:time($activity/endtime))}</endtime>
